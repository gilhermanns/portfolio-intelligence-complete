  ...
  return(
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${width} ${height}`} width="100%" style={{maxWidth:width,display:"block"}}>
        {yTicks.map(({y},i)=><line key={i} x1={PAD.left} x2={width-PAD.right} y1={y} y2={y} stroke="hsl(var(--border))" strokeWidth="0.5" strokeDasharray="4 3"/>)}
        ...
        {yTicks.map(({count,y},i)=><text key={i} x={PAD.left-4} y={y+3} fontSize="9" fill="hsl(var(--muted-foreground))" textAnchor="end">{count}</text>)}
